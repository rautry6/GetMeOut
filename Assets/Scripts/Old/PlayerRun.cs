...
{
    [Header("Assignable References")] [SerializeField]
    ...
    [Header("Adjustable Variables")] [SerializeField]
    private float groundCheckRayLength = 0.1f;

    [SerializeField] private float jumpForce;
    [SerializeField] private float jumpPressedRememberTime = 0.2f;
    [SerializeField] private float groundedRememberTime = 0.2f;
    [Range(5, 15)] [SerializeField] private float horizontalDamping;
    [Range(5, 15)] [SerializeField] private float airHorizontalDamping;
    ...
    void Update()
    {
        ...
        if (!IsGrounded())
        {
            ...
            playerCurrentHorizontalVelocity = _horizontal * airHorizontalDamping;
            playerRigidbody.velocity = new Vector2(playerCurrentHorizontalVelocity, playerRigidbody.velocity.y);
        }
        else
        {
            playerCurrentHorizontalVelocity = _horizontal * horizontalDamping;
        ...
        if (Input.GetButtonDown("Jump"))
        {
            ...
            if (_jumpPressedRemember > 0 && _groundedRemember > 0 || _doubleJump)
            {
